...
NOTIFY=`cat ${IN}config.json | jq ".job.notification_enable"`
...
do
    echo "$i %"
    PROGRESS=$(echo "scale=2;${i}/${DURATION}"|bc)
    if [ ${NOTIFY} = true ]; then
	curl -X POST -d '{"progress_value" : "'${PROGRESS}'", "progress_label" : "'${i}' / '${DURATION}'"}' ${NOTIFY_URL}
    fi
